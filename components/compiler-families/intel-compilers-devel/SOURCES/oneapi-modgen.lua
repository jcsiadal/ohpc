...
C.DEFAULTS = C.ONEAPIMODS .. "/.modulerc"
...
-- Split a string using a delimiter. Return as table
function SplitString(str, delim, maxsplit)
    local split = {}
    for s in str:gmatch("[^" .. delim .. "]+") do
        table.insert(split, s)
    end
    if maxsplit and (#split > maxsplit) then
        split[maxsplit] = table.concat(split, delim, maxsplit, #split)
        return { table.unpack(split, 1, maxsplit) }
    end
    return split
end
...
function ListDir(path, isRecursive, pattern, filetype)
    ...
    for entry in lfs.dir(path) do
        if entry ~= "." and entry ~= ".." then
            ...
            if (pattern == "" or pattern == nil or fnmatch.fnmatch(pattern, entry)) and
...
local defaultsfile = "]] .. PathCat(C.DEFAULTS, Defaults.xref[ver] .. ".lua") .. [["
...
-- Determine the closest version in the provided module list
local function closest_version(modlist, ver)
    local semver = function(v)
        local t = SplitString(v, ".", 3)
        if t[2] == nil then
            t[2] = 0
        end
        if t[3] == nil then
    ...
    local closest = ""
    local input = semver(ver)
    for k, v in pairs(modlist) do
        if closest == "" then
            closest = k
        end
        local current = semver(k)
        if current[1] == input[1] then
            if current[2] == input[2] then
                if current[3] == input[3] then
                    return k
                end
                if math.abs(current[3] - input[3]) < math.abs(semver(closest)[3] - input[3]) then
                    closest = k
                end
            end
            if math.abs(current[2] - input[2]) < math.abs(semver(closest)[2] - input[2]) then
                closest = k
            end
        end
        if math.abs(current[1] - input[1]) < math.abs(semver(closest)[1] - input[1]) then
            closest = k
        end
    end
end
...
-- Find OpenHPC LMOD modules in the default path
local function find_mods(modtype)
    local path = ModTypes[modtype].path
    local modlist = {}
    printformat("Searching for " .. ModTypes[modtype].name .. " modules.", "info")
    local foundfiles = ListDir(path, false, "*.lua", "file")
    if foundfiles == {} then
        printformat("No modules found", "step")
        return nil
    end
    for _, l in ipairs(foundfiles) do
...
local function find_tools(path, modtype)
    ...
    local isfull, isrt
    local ver
    for _, d in ipairs(ListDir(PathCat(path, mod.name), false, "2*", "directory")) do
        isfull = (lfs.attributes(PathCat(d, mod.find), "mode") == "file")
        isrt = (lfs.attributes(PathCat(d, mod.rt), "mode") == "file")
        ver = libgen.basename(d)
        if not isfull and isrt then
            toollist[ver] = "runtimes"
        end
        if isfull and isrt then
            toollist[ver] = "full version"
        end
        if isfull and not isrt then
            toollist[ver] = "incomplete (missing runtime)"
        end
        if not isfull and not isrt then
            toollist[ver] = "incomplete (missing components)"
        end
    end
    return toollist
end
...
local function oneapipath(path)
    ...
    if not path or path == "" then
        pathlist = { C.ONEAPIPATH }
        printformat("Using default path " .. pathlist[1], "info")
    else
        for _, p in ipairs(SplitString(pathlist, ":")) do
            isgood = true
            if not (dir_exists(p, false)) then
                printformat("Cannot find directory " .. p, "warning")
                isgood = false
            end
            if lfs.symlinkattributes(PathCat(p, C.MODSCRIPT), "mode") ~= "file" then
                printformat("Cannot find oneAPI module script in " .. p, "warning")
            end
            if isgood then
...
local function update_mods(modtype, path)
    printformat("Updating oneAPI modules ", "header")
    printformat("oneAPI modulefiles directory: " .. C.ONEAPIMODS, "info")
    ...
    if modtype == "compiler" then
        ...
        printformat(f, "step")
        WriteFile(f, moduletext("oneapi", "core", ""))
    end

    -- Regenerate the oneAPI modulefiles
    printformat("Regenerating " .. modtype .. " modulefile links", "info")
    for _, p in ipairs(oneapipath(path)) do
        printformat(p, "step")
        -- Run the oneAPI module script
        local _, iserr = RunOSCommand("cd " ..
            p .. " && . " .. PathCat(".", C.MODSCRIPT) .. C.SCRIPTOPTS .. "\'" .. tmpdir .. "\'")
        if iserr then
            printformat("Cannot update oneAPI modules", "warning")
        end
        ...
        for _, f in ipairs(filelist) do
            if libgen.basename(f) ~= "latest" then
                ...
                if newfile:find(PathCat(C.ONEAPIMODS, ModTypes[modtype].name) .. "/") ~= nil then
                    ...
                    WriteFile(modlist[ver].file, moduletext(modtype, ver, p))
                    printformat(modlist[ver].file, "substep")
                    if modtype == "compiler" then
                        WriteFile(PathCat(C.DEFAULTS, Defaults.xref[ver] .. ".lua"), moduletext("modulerc", ver, p))
                    end
                end
            end
        end
    ...
    if modlist == nil then
        printformat("No modules created", "warning")
        return nil
    else
        -- Create the default module link
...
local function new_default(modtype, version)
    if lfs.attributes(PathCat(ModTypes[modtype].path, version .. ".lua"), "mode") == "link" then
        if new_link(version .. ".lua", PathCat(ModTypes[modtype].path, "default")) then
            printformat("Default set to " .. version, "info")
        else
            printformat("Cannot set new default", "error")
        end
    else
        printformat("Module version " .. version .. " doesn't exist", "error")
    end
end
...
local function locate_all(modtype, path)
    ...
    printformat("Searching for " .. ModTypes[modtype].name .. " installations.", "info")
    local rootpath = ""
    for _, p in ipairs(oneapipath(path)) do
        printformat("Directory " .. p, "step")
        local toollist = find_tools(p, modtype)
        if toollist == {} then
            printformat("No modules found", "substep")
        else
            for k, v in pairs(toollist) do
                printformat("Found " .. v .. " " .. k, "substep")
            end
        end
    end
end
