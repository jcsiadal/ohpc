...
cat << EOF > %{buildroot}/%{modulepath}/%{version}%{OHPC_CUSTOM_PKG_DELIM}.lua
help([[
...
cat << EOF > %{buildroot}/%{modulepath}/.modulerc.lua
module_version("%{pname}/%{version}%{OHPC_CUSTOM_PKG_DELIM}", "default")
EOF
